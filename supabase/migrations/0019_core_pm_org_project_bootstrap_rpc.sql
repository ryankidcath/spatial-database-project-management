-- Fase 7 operasional: user baru bisa bootstrap organisasi + project pertama dari UI.
...
comment on function core_pm.create_organization_project_bootstrap(text, text, text, text, text) is
  'Bootstrap organisasi + project + status default + membership owner untuk user terautentikasi.';

revoke all on function core_pm.create_organization_project_bootstrap(text, text, text, text, text) from public;
grant execute on function core_pm.create_organization_project_bootstrap(text, text, text, text, text) to authenticated;
