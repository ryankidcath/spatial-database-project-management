...
import { validateDrawnLine } from "@/lib/workspace-map-draw-line";
import { pickDefaultVirtualTableLineMatchColumn } from "@/lib/virtual-table-geojson-import";
...
import { importVirtualRowsDrawnLineBatchAction } from "./virtual-table-actions";
import { emitVirtualTableRowsMutated } from "@/lib/workspace-virtual-table-mutations";

type Props = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  line: LatLngPoint[] | null;
...
export function WorkspaceSpatialDrawLineSaveDialog({
  open,
  onOpenChange,
  line,
  vtablesWithGeometry,
  ...
  useEffect(() => {
    ...
    const match = pickDefaultVirtualTableLineMatchColumn(matchColumns);
  ...
  const runSave = useCallback(() => {
    if (!line || line.length < 2) {
      setMsg("Garis gambar tidak valid.");
      return;
    }
    const validation = validateDrawnLine(line);
    if (!validation.ok) {
      setMsg(validation.error);
      return;
    }
    if (!tableId || !geometrySlug || !matchSlug || !matchKey.trim()) {
      setMsg("Isi tabel tujuan, kolom, dan kode garis.");
      return;
    }
    ...
    fd.set("line_json", JSON.stringify(line));
    ...
    startTransition(async () => {
      const r = await importVirtualRowsDrawnLineBatchAction(fd);
      if (r.error) {
        setMsg(r.error);
        return;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-md overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Simpan garis hasil gambar</DialogTitle>
          <DialogDescription>
            LineString WGS84 dari peta akan disimpan ke tabel virtual (mis.
            Jalan, Saluran). Cocokkan dengan sketsa kertas dan titik T1…Tn.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-3 text-sm">
          <div>
            <Label htmlFor="draw-line-save-table">Tabel tujuan</Label>
            <select
              id="draw-line-save-table"
              value={tableId}
          ...
          <div>
            <Label htmlFor="draw-line-save-key">Kode garis / kunci</Label>
            <Input
              id="draw-line-save-key"
              value={matchKey}
              onChange={(e) => setMatchKey(e.target.value)}
              placeholder="contoh: JLN-01 atau SAL-Utara"
          ...
          <div>
            <Label htmlFor="draw-line-save-label">Judul (opsional)</Label>
            <Input
              id="draw-line-save-label"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder={`Garis ${matchKey || "…"}`}
              className="mt-1"
              disabled={pending}
            />
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div>
              <Label htmlFor="draw-line-save-geom">Kolom geometri</Label>
              <select
                id="draw-line-save-geom"
                value={geometrySlug}
            ...
            <div>
              <Label htmlFor="draw-line-save-match-col">Kolom kunci</Label>
              <select
                id="draw-line-save-match-col"
                value={matchSlug}
          ...
          {relationColumns.length > 0 ? (
            <div className="space-y-2 rounded-md border border-border bg-muted/20 p-3">
              <div>
                <Label htmlFor="draw-line-save-rel">Kolom relasi</Label>
                <select
                  id="draw-line-save-rel"
          ...
          <div>
            <Label htmlFor="draw-line-save-upsert">Mode</Label>
            <select
              id="draw-line-save-upsert"
              value={upsertMode}
              onChange={(e) =>
